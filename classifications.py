...
dataset = pd.read_csv('only_road_accidents_data_month2.csv')
...
#imputer = imputer.fit(X[:, 2:13])
#X[:, [2,13]] = imputer.transform(X[:, [2,13]])
...
plt.xlabel('Months')
plt.ylabel('Estimated Suicide Ratio')
plt.legend()
plt.show()
